...
// END PARSER
  ...
  var compiler = (function () {
    ...
    Compiler.OPCODE_PARAM_LENGTH_MAP = {
      ...
      op_appendContent: 1,
      op_applyProcessor: 1,
      op_applyPrePipeline: 0,
      op_applyPostPipeline: 0,
      op_escape: 0,
    ...
    Compiler.prototype = {
      ...
      pushOpcode: function (name, param1) {
      ...
      type_mustache: function (node, next) {
        ...
        this.type_name(name);
        this.pushOpcode('op_evaluateValue', name.path);
        this.pushOpcode('op_applyPrePipeline');
        for (i = 0; i < len; i++) {
          this.pushOpcode('op_applyProcessor', processors[i]);
        }
        this.pushOpcode('op_applyPostPipeline');
        if (node.escape) {
    ...
    JsCompiler.prototype = {
      ...
      generate: function (subPrograms, asObject) {
        var body;
        this.source[0] = this.source[0] + 'var tmp, buffer = "", processor, valueInfo, index, hasNext, contextStack = [context], ' +
          ...
          'prePipeline = this.prePipeline, postPipeline = this.postPipeline, processors = this.processors;' +
          '\n\n' + subPrograms.join('\n\n') + '\n';
      ...
      generateSubProgram: function () {
        var body;
        var indent = '  ';
        this.source[0] = this.source[0] + 'var tmp, buffer = "", processor, valueInfo, index, hasNext;';
        this.source.push('return buffer;');
      ...
      op_applyProcessor: function (processorName) {
        this.source.push('processor = ' + this.lookup('context', processorName) + ';');
        this.source.push('if (typeof processor === "function") { tmp = processor.call(context, tmp, valueInfo); }');
        this.source.push('else { processor = ' + this.lookup('processors', processorName) + ';');
        this.source.push('  if (typeof processor === "function") { tmp = processor.call(context, tmp, valueInfo); }');
        this.source.push('  else { tmp = noSuchProcessor.call(context, "' + processorName + '", tmp, valueInfo); }}');
      },

      op_applyPrePipeline: function () {
        this.source.push('tmp = prePipeline.call(context, tmp, valueInfo);');
      },

      op_applyPostPipeline: function () {
        this.source.push('tmp = postPipeline.call(context, tmp, valueInfo);');
      },
      ...
      op_evaluateValue: function (name) {
        this.source.push('valueInfo = {name: "' + name + '", index: index, hasNext: hasNext};');
        this.source.push('if (typeof tmp === "function") { tmp = tmp.call(context); }');
        this.source.push('else if (tmp === void 0) { tmp = noSuchValue.call(context, valueInfo); }');
      },
  ...
  var tempura =  (function () {
    ...
    return {
      ...
      settings: {
        ...
        prePipeline: function (value, valueInfo) {
          return value;
        },

        postPipeline: function (value, valueInfo) {
          return value == null ? '': value;
        },

        noSuchValue: function (valueInfo) {
          return;
        },

        noSuchProcessor: function (processorName, value, valueInfo) {
          return value;
        }
      },
